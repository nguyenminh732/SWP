...
<html lang="en">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const buttons = document.querySelectorAll(".add-to-menu-btn");
            buttons.forEach(button => {
                button.addEventListener("click", function () {
                    const menuId = this.getAttribute("data-menu-id");
                    const tableId = new URLSearchParams(window.location.search).get('tbid'); // Lấy ID bàn từ URL

                    if (!tableId) {
                        alert("Không tìm thấy số bàn!");
                        return;
                    }
                    ...
                    fetch("AddToCartServlet", {
                        ...
                        body: `menuId=${menuId}&tableId=${tableId}`
                    })
